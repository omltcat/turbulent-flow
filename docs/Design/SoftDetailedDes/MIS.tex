...
\subsection{Uses}
\begin{itemize}
\item Parse query input to call flow field module, serialize and return the result
\item Queue multiple queries
\end{itemize}
...
\subsection{Uses}
\begin{itemize}
\item (Currently) Load eddy profile defined by the user [SRS: A3].
\item (Future) Generate eddy profile based on user input parameters [MG: AC1].
\end{itemize}
...
\subsection{Uses}
\begin{itemize}
\item Create flow field based on eddy profile or load existing one.
\item Compute velocity vector at given position and time.
\end{itemize}
...
\subsection{Uses}
\begin{itemize}
\item Create eddy object based on parameters given.
\item Compute velocity vector relative position from center.
\end{itemize}
...
\subsection{Uses}
\begin{itemize}
\item A library of shape functions to be used by \texttt{eddy} (\ref{mEddy}).
\end{itemize}
...
\subsection{Uses}
\begin{itemize}
\item Take command line arguments and call various modules accordingly.
\end{itemize}
...
\subsection{Uses}
\begin{itemize}
\item Read and write JSON files for eddy profile and flow field.
\end{itemize}
...
\subsection{Uses}
\begin{itemize}
\item Render 2D or 3D images of the flow field.
\end{itemize}
