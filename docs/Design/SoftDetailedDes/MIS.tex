...
\begin{itemize}
  ...
  \item \textbf{EddyT}: Eddy object, stores eddy intensity, length\_scale orientation and position information, with methods for velocity calculation, see Eddy Module [MIS\ref{mEddy}].
...
\begin{itemize}
  ...
  \end{itemize}
  \item exception: exc := (\texttt{profile\_name}, \texttt{field\_name}, \texttt{dimensions} not inputted $\Rightarrow$ ``Required arguments not inputted"). \texttt{avg\_vel} has default value of 0.0 so it can be omitted.
  \item exception: exc := (Any arguments inputted other than \texttt{profile\_name}, \texttt{field\_name}, \texttt{dimensions}, \texttt{avg\_vel} $\Rightarrow$ ``Unrecognized arguments")
\end{itemize}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item \texttt{console} := \texttt{query.handle\_request(field, query\_file)} to get the velocity vectors at the queried positions and times specified in \texttt{query\_file}, and output an operation summary (where the raw result and plot are saved).
  \end{itemize}
  \item exception: exc := (\texttt{field\_name}, \texttt{query\_file} not inputted $\Rightarrow$ ``Required arguments not inputted"). 
  \item exception: exc := (Any arguments inputted other than \texttt{field\_name}, \texttt{query\_file}, \texttt{shape\_func}, \texttt{cutoff} $\Rightarrow$ ``Unrecognized arguments")
\end{itemize}
